...
public class KerfuEntity extends PathfinderMob implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(KerfuEntity.class, EntityDataSerializers.BOOLEAN);
	...
	public KerfuEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ThisnotamodModEntities.KERFU.get(), world);
	}

	public KerfuEntity(EntityType<KerfuEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			private final Mob mob = (Mob) KerfuEntity.this;
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("fix server"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(KerfuEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
