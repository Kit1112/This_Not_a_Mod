...
public class FridgeDownTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	...
	public int blockstateNew = this.getBlockState().getValue(FridgeDownBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(FridgeDownBlock.BLOCKSTATE);

	public FridgeDownTileEntity(BlockPos pos, BlockState state) {
		super(ThisnotamodModBlockEntities.FRIDGE_DOWN.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(FridgeDownBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(FridgeDownBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<FridgeDownTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<FridgeDownTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public Component getDefaultName() {
		return Component.literal("fridge_down");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new FridgeDownIntMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
