...
public class TerminalTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(TerminalBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(TerminalBlock.BLOCKSTATE);

	public TerminalTileEntity(BlockPos pos, BlockState state) {
		super(ThisnotamodModBlockEntities.TERMINAL.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(TerminalBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(TerminalBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<TerminalTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<TerminalTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
