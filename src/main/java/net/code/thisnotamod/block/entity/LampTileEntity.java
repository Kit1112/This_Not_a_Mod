...
public class LampTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(LampBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(LampBlock.BLOCKSTATE);

	public LampTileEntity(BlockPos pos, BlockState state) {
		super(ThisnotamodModBlockEntities.LAMP.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(LampBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(LampBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<LampTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<LampTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
