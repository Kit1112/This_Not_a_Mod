...
public class ServerTopTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(ServerTopBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(ServerTopBlock.BLOCKSTATE);

	public ServerTopTileEntity(BlockPos pos, BlockState state) {
		super(ThisnotamodModBlockEntities.SERVER_TOP.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(ServerTopBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(ServerTopBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<ServerTopTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<ServerTopTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public Component getDefaultName() {
		return Component.literal("server_top");
	}
