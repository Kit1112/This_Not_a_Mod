...
public class ShopIntTestTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ShopIntTestTileEntity(BlockPos pos, BlockState state) {
		super(ThisnotamodModBlockEntities.SHOP_INT_TEST.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ShopIntTestBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<ShopIntTestTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<ShopIntTestTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public Component getDefaultName() {
		return Component.literal("shop_int_test");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new DeliveryGuiMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}

	@Override
	public Component getDisplayName() {
		return Component.literal("Тестовый блок для использования магазина");
	}
