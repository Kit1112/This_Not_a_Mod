...
public class ZigaTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ZigaTileEntity(BlockPos pos, BlockState state) {
		super(ThisnotamodModBlockEntities.ZIGA.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ZigaBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<ZigaTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<ZigaTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
